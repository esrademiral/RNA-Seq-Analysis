...
working_dir = "/home/esra/workspace/rnaseq/de/ballgown/ref_only_my_data" 
setwd(working_dir)
...
gene_expression[1:3,c("FPKM.SRR7618907_expression","FPKM.SRR7618908_expression","FPKM.SRR7618909_expression","FPKM.SRR7618904_expression")]
...
#View expression values for the transcripts of a particular gene symbol of chromosome 22.  e.g. 'EGR1'
#First determine the rows in the data.frame that match 'EGR1', aka. ENSG00000120738, then display only those rows of the data.frame
i = row.names(gene_expression) == "ENSG00000120738"
gene_expression[i,]

#What if we want to view values for a list of genes of interest all at once? 
#genes_of_interest = c("EGR1 "HNRNPA1P4", "TENT5C", "ASNS")
genes_of_interest = c("ENSG00000120738","ENSG00000206228","ENSG00000183508","ENSG00000070669")
...
min(gene_expression[,"FPKM.SRR7618907_expression"])
max(gene_expression[,"FPKM.SRR7618907_expression"])
...
data_columns=c(1:6)
short_names=c("Gefitinib_1","Gefitinib_2","Gefitinib_3","DMSO_1","DMSO_2","DMSO_3")
...
x = gene_expression[,"FPKM.SRR7618907_expression"]
y = gene_expression[,"FPKM.SRR7618908_expression"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (Gefitinib, Replicate 1)", ylab="FPKM (Gefitinib, Replicate 2)", main="Comparison of expression values for a pair of replicates")
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (Gefitinib, Replicate 1)", ylab="FPKM (Gefitinib, Replicate 2)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
plotCor("FPKM.SRR7618907_expression", "FPKM.SRR7618904_expression", "Gefitinib_1 vs DMSO_1", "tomato2")
plotCor("FPKM.SRR7618908_expression", "FPKM.SRR7618905_expression", "Gefitinib_2 vs DMSO_2", "royalblue2")
...
par(mfrow=c(1,2))
plotCor2("FPKM.SRR7618907_expression", "FPKM.SRR7618904_expression", "Gefitinib_1 vs DMSO_1", "tomato2")
plotCor2("FPKM.SRR7618908_expression", "FPKM.SRR7618905_expression", "Gefitinib_2 vs DMSO_2", "royalblue2")
...
sig=which(results_genes$pval<0.01)
results_genes[,"de"] = log2(results_genes[,"fc"])
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) Gefitinib vs DMSO", main="Distribution of differential expression values")
...
gene_expression[,"Gefitinib"]=apply(gene_expression[,c(1:3)], 1, mean)
gene_expression[,"DMSO"]=apply(gene_expression[,c(4:6)], 1, mean)

x=log2(gene_expression[,"Gefitinib"]+min_nonzero)
y=log2(gene_expression[,"DMSO"]+min_nonzero)

plot(x=x, y=y, pch=16, cex=0.25, xlab="Gefitinib FPKM (log2)", ylab="DMSO FPKM (log2)", main="Gefitinib vs DMSO FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
...
text(x[topn], y[topn], results_genes[topn,"gene_name"], col="blue", cex=0.75, srt=45)
...
results_genes$diffexpressed[results_genes$de > 0.6 & results_genes$pval < 0.01] <- "Up"
# if log2Foldchange < -2 and pvalue < 0.05, set as "Down regulated"
results_genes$diffexpressed[results_genes$de < -0.6 & results_genes$pval < 0.01] <- "Down"
